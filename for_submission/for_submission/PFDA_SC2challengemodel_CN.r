clin <- read.table("data_files\\sc2_Phase1_CN_Outcome.tsv", sep="\t", header=TRUE, stringsAsFactors=FALSE)
phen <- read.table("data_files\\sc2_Phase1_CN_Phenotype.tsv", sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
glmcn <- glm(f1, data=clinphen, family=gaussian())
...
##correlation of significant phenotypes with CN
dataexp <- read.table("data_files\\sc2_Phase1_CN_FeatureMatrix.tsv", sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
corgecut <- subset(as.data.frame(corge), (SURVIVAL_STATUS > 0.2 | SURVIVAL_STATUS < -0.2 | CancerType_GBM > 0.2 | CancerType_GBM < -0.2) & ((SURVIVAL_STATUS > 0 & CancerType_GBM > 0) | (SURVIVAL_STATUS < 0 & CancerType_GBM < 0)))
...
write.table(subset(as.data.frame(pathpar), s0!=0),"sc2_Phase1_CN_ElNet_parameters.txt", sep="\t", quote=FALSE)
...
png("ElNet_Sc2CNmodelROC_Allsampletest.png", width=2800, height=2600, res=300)
plot(elnetroc)
dev.off()
...
write.table(predictclass, "sc2_Phase1_CN_predictions.txt", sep="\t", quote=FALSE, row.names=FALSE)
...
write.table(confusion, "sc2_Phase1_CN_confusion-matrix.txt", sep="\t", quote=FALSE)
