clin <- read.table("data_files\\sc1_Phase1_GE_Outcome.tsv", sep="\t", header=TRUE, stringsAsFactors=FALSE)
phen <- read.table("data_files\\sc1_Phase1_GE_Phenotype.tsv", sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
clinphen[,6] <- clinphen[,6] + clinphen[,10]
clinphen[,7] <- clinphen[,7] + clinphen[,10]
clinphen[,8] <- clinphen[,8] + clinphen[,10]
clinphen[,9] <- clinphen[,9] + clinphen[,10]
clinphen[,11] <- clinphen[,11] + clinphen[,10]
clinphen[,12] <- clinphen[,12] + clinphen[,10]
clinphen[,13] <- clinphen[,13] + clinphen[,10]
clinphen[,14] <- clinphen[,14] + clinphen[,17]
clinphen[,15] <- clinphen[,15] + clinphen[,17]
clinphen[,16] <- clinphen[,16] + clinphen[,17]
clinphen[,18] <- clinphen[,18] + clinphen[,17]
#Get rid of NAs
clinphen <- clinphen[,-c(5,10,17)]
for(i in 1:ncol(clinphen))
clinphen[,i] <- as.numeric(clinphen[,i])
...
glmge <- glm(f1, family=gaussian(), data=clinphen)
siggenes <- rownames(subset(as.data.frame(summary(glmge)[12]), coefficients.Pr...t.. < 0.1))[-1]
...
dataexp <- read.table("data_files\\sc1_Phase1_GE_FeatureMatrix.tsv", sep="\t", header=TRUE, stringsAsFactors=FALSE)
...
corgecut <- subset(as.data.frame(corge), (SURVIVAL_STATUS > 0.3 | SURVIVAL_STATUS < -0.3 | CancerType_GBM > 0.3 | CancerType_GBM < -0.3) & ((SURVIVAL_STATUS > 0 & CancerType_GBM > 0) | (SURVIVAL_STATUS < 0 & CancerType_GBM < 0)))
...
#In the following steps we will try address the class imbalance problem by matching the number of samples in each of the two groups (survival status 0 or 1) in the training set
#Get the number of samples in each category of survival status (0 or 1) from the training set
...
##Do not run..The following lines describe the model training and parameter optimization.. The optimized pre-built model is saved as GEmodel.RData (that we loaded below for testing)
#a <- seq(0.1, 0.9, 0.05)
...
data(GEmodel)
##get the coefficients of the parameters (phenotype + gene expression) used in the elastic net model
pathselect <- coef(md3)
pathpar <- as.matrix(pathselect)
##get the parameters with non-zero coefficients from the elastic net model
selectlinc <- unique(rownames(subset(as.data.frame(pathpar), s0!=0))[-1])
write.table(subset(as.data.frame(pathpar), s0!=0),"sc1_Phase1_GE_ElNet_parameters.txt", sep="\t", quote=FALSE)
##Predict the survival status from all samples
predicttest <- as.numeric(predict(md3, as.matrix(clindataexp[,2:ncol(clindataexp)]), type = "class")[,1])
classtest <- as.numeric(clindataexp[,1])
elnetroc <- roc(as.numeric(classtest), as.numeric(predicttest))
elnetauc <- elnetroc$auc
##Plot the ROC for the testing set
png("ElNet_Sc1GEmodelROC_Allsampletest.png", width=2800, height=2600, res=300)
plot(elnetroc)
dev.off()
...
write.table(predictclass, "sc1_Phase1_GE_predictions.txt", sep="\t", quote=FALSE, row.names=FALSE)
...
write.table(confusion, "sc1_Phase1_GE_confusion-matrix.txt", sep="\t", quote=FALSE)
